...
# -*- coding: utf-8 -*-
"""
Created on Mon Sep 16 19:24:27 2019
...
#            if self.partial_labels[i] == 1:
#                clamp_data_label[i][0] = 1
#            else:
#                clamp_data_label[i][1] = 1
#            clamp_data_label[i][self.Y[i]] = 1
...
#        outputs = 1 / (1 + np.exp(-outputs))
    ...
    def calc_accuracy(self):
        total = len(self.unlabel_idx)
        accu = np.where(self.true_labels[self.unlabel_idx] == self.unlabel_data_labels)[0].shape[0]
        print("total unlabeled,", total, "accu:", accu)
        return accu/total
        
    def build_graph_using_LLE(self):
        my_LLE = LocallyLinearEmbedding(n_neighbors = self.n_neighbors, n_components = 2)
        my_LLE.fit(self.data)
        return my_LLE.get_LLE_weight_matrix(self.data)
...
class LocallyLinearEmbedding(BaseEstimator, TransformerMixin):
    ...
    def get_LLE_weight_matrix(self, X):
        """
        Transform new points into embedding space.
