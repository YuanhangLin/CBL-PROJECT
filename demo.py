...
class Graph_Convolution_With_MLP(nn.Module):
    
    def __init__(self):
        
        super(Graph_Convolution_With_MLP, self).__init__()
        self._spectra_gc = nn.Linear()
        self._thermal_gc = nn.Linear()
...
def demo():
    train_date = "130411"
    test_date = "130411"
    make_dataset_aggregated_polygon(train_date)
    if test_date != train_date:
        make_dataset_aggregated_polygon(test_date)
